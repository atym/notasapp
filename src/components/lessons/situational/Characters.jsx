...
import { FiUser, FiCalendar, FiMapPin, FiFlag, FiMessageCircle, FiChevronDown, FiChevronRight } from 'react-icons/fi';
...
const CharacterLesson = () => {
    ...
    const questions = [
        { id: 'name', q: '¿Cómo se llama?', a: `Se llama ${person.firstName}.`, icon: <FiUser size={18}/> },
        { id: 'lastname', q: '¿Cómo se apellida?', a: `Se apellida ${person.lastName}.`, icon: <FiUser size={18}/> },
        { id: 'age', q: '¿Cuántos años tiene?', a: `Tiene ${person.age} años.`, icon: <FiCalendar size={18}/> },
        { id: 'origin', q: '¿De dónde es?', a: `Es de ${person.origin}.`, icon: <FiMapPin size={18}/> },
        { id: 'nat', q: '¿Cuál es su nacionalidad?', a: `Es ${person.nationality}.`, icon: <FiFlag size={18}/> },
        { id: 'lang', q: '¿Qué idioma habla?', a: `Habla ${person.language}.`, icon: <FiMessageCircle size={18}/> },
    ];
        ...
        <div className="flex flex-col h-full max-w-md mx-auto p-4 pt-0">
                ...
                {questions.map((item) => (
                    <div key={item.id} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
                        <button onClick={() => toggleReveal(item.id)} className="w-full flex items-center justify-between p-4 text-left active:bg-gray-700 transition-colors">
                            ...
                            <FiChevronDown size={16} className={`text-gray-500 transition-transform ${revealed[item.id] ? 'rotate-180' : ''}`} />
                        </button>
            ...
            <div className="fixed bottom-6 left-0 right-0 px-6 max-w-md mx-auto">
                <button onClick={nextPerson} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl shadow-lg shadow-indigo-900/50 flex items-center justify-center gap-2 active:scale-95 transition-all">
                    {index === characters.length - 1 ? "Terminar Lección" : "Siguiente Personaje"} <FiChevronRight size={20} />
                </button>
            </div>
        </div>
    );
